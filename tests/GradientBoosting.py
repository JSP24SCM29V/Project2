...
# class DecisionTree:
#     def __init__(self, max_depth=4, min_samples_split=2, min_impurity_decrease=0.0):
#         self.max_depth = max_depth
#         self.min_samples_split = min_samples_split
...
#                 right_loss = self._mse(y[right_idx])
#                 weighted_loss = (np.sum(left_idx) * left_loss + np.sum(right_idx) * right_loss) / m
#                 loss_reduction = current_loss - weighted_loss

#                 if weighted_loss < best_loss and loss_reduction >= self.min_impurity_decrease:
#                     best_loss = weighted_loss
#                     best_feature = feature
#                     best_threshold = threshold
...
# class GradientBoostingClassifier:
#     def __init__(self, n_estimators=100, learning_rate=0.1, max_depth=4, min_samples_split=2, min_impurity_decrease=0.0, subsample=0.8, max_features=0.8):
#         self.n_estimators = n_estimators
#         self.learning_rate = learning_rate
#         self.max_depth = max_depth
#         self.min_samples_split = min_samples_split
...
#         self.trees = []
#         self.initial_pred = None
#         self.alpha = 1.0  # Scaling factor for log-odds
#         self.beta = 0.0   # Shift factor for log-odds
#         self.y_train_ = None
...
#     def _calibrate_sigmoid(self, X, y, predictions, max_iter=100, lr=0.1):
#         """Optimize alpha and beta to minimize log loss using gradient descent."""
#         alpha, beta = 1.0, 0.0
#         y = (y + 1) / 2  # Convert {-1, 1} to {0, 1} for log loss
#         for _ in range(max_iter):
#             # Compute probabilities with current alpha, beta
#             scaled_pred = alpha * predictions + beta
#             proba = self._sigmoid(scaled_pred)
            
#             # Compute gradients
#             error = proba - y
#             grad_alpha = np.mean(error * predictions)
#             grad_beta = np.mean(error)
...
#             if max(abs(grad_alpha), abs(grad_beta)) < 1e-4:
#                 break
...
#         # Validation set for early stopping
#         val_idx = np.random.choice(n_samples, int(0.05 * n_samples), replace=False)
#         train_idx = np.setdiff1d(np.arange(n_samples), val_idx)
#         X_train, y_train = X[train_idx], y[train_idx]
#         X_val, y_val = X[val_idx], y[val_idx]
...
#         patience = 20
#         patience_counter = 0
#         sample_idx_cache = None

#         for i in range(self.n_estimators):
#             # Resample every 10 iterations
#             if i % 10 == 0:
#                 sample_idx_cache = np.random.choice(len(X_train), int(self.subsample * len(X_train)), replace=False)
#             X_sample, y_sample = X_train[sample_idx_cache], y_train[sample_idx_cache]
...
#             probabilities = self._sigmoid(predictions[train_idx][sample_idx_cache])
#             gradients = y_sample * (1 - probabilities)
...
#             if i % 10 == 0 and len(val_idx) > 0:
#                 val_proba = self._sigmoid(predictions[val_idx])
#                 val_loss = self._log_loss((y_val + 1) / 2, val_proba)
#                 if val_loss < best_val_loss:
#                     best_val_loss = val_loss
#                     patience_counter = 0
#                 else:
...
#         self.alpha, self.beta = self._calibrate_sigmoid(X, y, predictions)
...
#         probabilities = self.model._sigmoid(self.model.alpha * predictions + self.model.beta)
#         return np.vstack((1 - probabilities, probabilities)).T
